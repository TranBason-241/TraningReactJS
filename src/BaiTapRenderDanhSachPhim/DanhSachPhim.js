...
import style from './DanhSachPhim.module.css'
const listMovie = [
    {
      "maPhim": 1321,
      "tenPhim": "Ted2",
...
export default class DanhSachPhim extends Component {
    ...
    RenderMovie = () =>{
        ...
        const view = listMovie.map((movie,index) => {
            return <div className="col-4">
            <div  className="card" style={{width: '18rem'}} >
            <img style={{ width: "100%", height: "150px"}} src={movie.hinhAnh} className="card-img-top " alt="..." />
            <div className="card-body">
        <h5 style={{height: "50px"}} className="card-title">{movie.tenPhim.length>20 ? <span>{movie.tenPhim.substr(0,20)} ...</span> : <span>{movie.tenPhim}</span>}</h5>
        <p style={{height:"130px"}} className="card-text">{movie.moTa.length > 150 ? <span> {movie.moTa.substr(0,100)}... </span> : <span>{movie.moTa}</span> }</p>
                
                <p className="card-text">{movie.ngayKhoiChieu}</p>
                <a href={movie.trailer} className="btn btn-primary">Traier</a>
            </div>
            </div>
            </div> 
